...
_Locked 2026-07-02, **open-core model revised 2026-07-03 (Ryan): full Cal.com-style open source — one repo, backend included.** **Relicensed 2026-07-09 (Ryan, post-launch): AGPLv3 → MIT — maximum openness chosen over the copyleft guardrail; AGPL mentions below record the earlier decision.** **Collaboration re-scoped 2026-07-03 (Ryan, via design interview): two features — (A) shared workspaces of up to 4 (owner + 3), and (B) per-note publishing via public link.** How multi-device sync, shared workspaces, note publishing, and the open-vs-paid model fit together._
