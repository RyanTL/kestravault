...
- [x] **Open-core = full Cal.com model (revised 2026-07-03):** one public repo, everything (apps + `core` + `supabase/` backend + billing) **AGPLv3** — no repo split, no closed backend. MIT→AGPLv3 relicense **done** (dependency-license audit → swapped `LICENSE` + `package.json` license fields + README/CONTRIBUTING). See [sync-collab-open-core.md](sync-collab-open-core.md). _Remaining: `NOTICE` line for the bundled proprietary `@anthropic-ai/claude-agent-sdk`._
